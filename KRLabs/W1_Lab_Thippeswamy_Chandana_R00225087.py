...
#Initialising the Vacuum agent
ag = TableDrivenVacuumAgent()

#Initialise the environment
env = TrivialVacuumEnvironment()
print('Initialising the Vacuum enviornment with status:', env.status)
print('-------------------------------------')

#Adding agent to the Vacuum environment
env.add_thing(ag)

print('Vacuum Environment after adding the agent:', env.status)
print('-------------------------------------')

print('Vacuum Environment after running the agent')

#Running the Vacuum agent for 2 runs
env.run(2)
print('-------------------------------------')

#Running the Vacuum agent for 4 runs
env.run(4)
print('-------------------------------------')

#Running the Vacuum agent for 8 runs
env.run(8)
print('-------------------------------------')
...
print('\n Farmer Dilemma Solution \n')

def TableDrivenFarmerAgent():
    """Tabular approach towards Farmer's world 
    """
    table = {((loc_A, 'Safe'),): 'Right',
             ((loc_A, 'Danger'),): 'PickChicken',
             ((loc_B, 'Safe'),): 'Left',
             ((loc_B, 'Danger'),): 'PickFox',
             ((loc_A, 'Danger'), (loc_A, 'Safe')): 'Right',
             ((loc_A, 'Safe'), (loc_B, 'Danger')): 'PickFox',
             ((loc_B, 'Safe'), (loc_A, 'Danger')): 'PickChicken',
             ((loc_B, 'Danger'), (loc_B, 'Safe')): 'Left',
             ((loc_A, 'Danger'), (loc_A, 'Safe'), (loc_B, 'Danger')): 'PickFox',
             ((loc_B, 'Danger'), (loc_B, 'Safe'), (loc_A, 'Danger')): 'PickChicken'}
    return Agent(TableDrivenAgentProgram(table))
...
class FarmerEnvironment(XYEnvironment):
    """The environment of [Ex. 2.12]. Agent perceives dirty or clean,
    and bump (into obstacle) or not; 2D discrete world of unknown size;
    performance measure is 100 for each dirt cleaned, and -1 for
    each turn taken."""
    ...
    def thing_classes(self):
        return [Obs, Obj, TableDrivenFarmerAgent]

    def percept(self, agent):
        """The percept is a tuple of ('Danger' or 'Safe', 'Bump' or 'None').
        """
        status = ('Danger' if self.some_things_at(
            agent.location, Obj) else 'Safe')
        bump = ('Bump' if agent.bump else 'None')
        return status, bump
    ...
    def execute_action(self, agent, action):
        agent.bump = False
        if action == 'Suck':
            obj_list = self.list_things_at(agent.location, Obj)
            if obj_list != []:
                obj = obj_list[0]
                agent.performance += 100
                self.delete_thing(obj)
        else:
            super().execute_action(agent, action)
...
class TrivialFarmerEnvironment(Environment):
    """This environment has two locations, A and B. Each can be Safe
    or Danger. The agent perceives its location and the location's
    status. This serves as an example of how to implement a simple
    Environment."""

    def __init__(self):
        super().__init__()
        self.status = {loc_A: random.choice(['Safe', 'Danger']),
                       loc_B: random.choice(['Safe', 'Danger'])}

    def thing_classes(self):
        return [Obs, Obj, TableDrivenFarmerAgent]

    def percept(self, agent):
        """Returns the agent's location, and the location status (Danger/Safe)."""
        return agent.location, self.status[agent.location]
    ...
    def execute_action(self, agent, action):
        ...
        if action == 'Right':
            ...
            agent.location = loc_A
            agent.performance -= 1
        elif action == 'PickChicken':
            if self.status[agent.location] == 'Danger':
                agent.performance += 10
        elif action == 'PickFox':
            if self.status[agent.location] == 'Danger':
                agent.performance += 10
            self.status[agent.location] = 'Safe'

    def default_location(self, thing):
        """Agents start in either location at random."""
        return random.choice([loc_A, loc_B])

#Initialising farmer agent
farmeragent = TableDrivenFarmerAgent()

#Initialise the farmer environment
farmerenv = TrivialFarmerEnvironment()
print('Initialising the Farmer enviornment with status:', farmerenv.status)

#Adding farmer agent to farmer environment
farmerenv.add_thing(farmeragent)

print('Farmer Environment after adding farmer agent:', farmerenv.status)

#Running farmer agent
farmerenv.run()

print('Farmer Environment after running farmer agent:', farmerenv.status)
